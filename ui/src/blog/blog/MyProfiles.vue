<template>
  ...
  <div v-else style="display: flex; flex-direction: column">
    <span v-if="error">Error fetching the profiles: {{error.data.data}}.</span>
    <div v-else-if="hashes && hashes.length > 0" style="margin-bottom: 8px">
      <ProfileDetail 
        v-for="hash in hashes" 
        :profile-hash="hash"
        @profile-deleted="fetchProfile()"
      >
      </ProfileDetail>
    </div>
    <span v-else>No profiles found for this author.</span>
  </div>
...
export default defineComponent({
  ...
  async mounted() {
    if (this.author === undefined) {
      throw new Error(`The author property is required for the MyProfiles element`);
    }
