...
import { createListing, sampleListing } from './common.js';
...
test('create Listing', async () => {
  await runScenario(async scenario => {
    ...
    const record: Record = await createListing(alice.cells[0]);
    assert.ok(record);
  });
});

test('create and read Listing', async () => {
  await runScenario(async scenario => {
    // Construct proper paths for your app.
    // This assumes app bundle created by the `hc app pack` command.
    const testAppPath = process.cwd() + '/../workdir/healthcare_forum.happ';
    ...
    const sample = await sampleListing(alice.cells[0]);

    // Alice creates a Listing
    const record: Record = await createListing(alice.cells[0], sample);
    assert.ok(record);
...
test('create and update Listing', async () => {
  await runScenario(async scenario => {
    // Construct proper paths for your app.
    // This assumes app bundle created by the `hc app pack` command.
    const testAppPath = process.cwd() + '/../workdir/healthcare_forum.happ';
    ...
    let contentUpdate: any = await sampleListing(alice.cells[0]);
    let updateInput = {
    ...
    contentUpdate = await sampleListing(alice.cells[0]);
    updateInput = { 
      original_listing_hash: originalActionHash,
...
test('create and delete Listing', async () => {
  await runScenario(async scenario => {
    // Construct proper paths for your app.
    // This assumes app bundle created by the `hc app pack` command.
    const testAppPath = process.cwd() + '/../workdir/healthcare_forum.happ';
