...
import { createProfile } from './common.js';

test('create a Profile and get all profiles', async () => {
  await runScenario(async scenario => {
    ...
    const createdRecord: Record = await createProfile(alice.cells[0]);
    assert.ok(createdRecord);
    
    await pause(1200);
    
    // Bob gets all profiles again
