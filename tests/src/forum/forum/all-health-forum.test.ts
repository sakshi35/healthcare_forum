...
import { createHealthForum } from './common.js';

test('create a HealthForum and get all health forum', async () => {
  await runScenario(async scenario => {
    ...
    const createdRecord: Record = await createHealthForum(alice.cells[0]);
    assert.ok(createdRecord);
    
    await pause(1200);
    
    // Bob gets all health forum again
